...
static struct event *server_time_ev = NULL;
static struct timeval server_time_update_timeout = {0, 400 * 1000};
static struct timeval server_time;
static char server_fmttime[TIMEBUF_LEN];

static void server_time_update_cb(evutil_socket_t fd, short events, void *arg)
{
	struct tm	*tm;
	time_t prev = server_time.tv_sec;

	gettimeofday(&server_time, NULL);
	tm=gmtime(&(server_time.tv_sec));
	strftime(server_fmttime, TIMEBUF_LEN, "%Y-%m-%d %H:%M:%S %Z", tm);

	event_add(server_time_ev, &(server_time_update_timeout));
}

const char * http_server_fmttime()
{
	return (const char *)server_fmttime;
}
...
struct http_server * http_server_new(struct event_base *base, int port)
{
	...
	/* add a server time update event */
	if (!server_time_ev) {
		server_time_ev = event_new(base, -1, EV_TIMEOUT,
			server_time_update_cb, NULL);
		if (!server_time_ev) {
			printf("Could not initial server time\n");
			goto failed;
		}
		event_add(server_time_ev, &server_time_update_timeout);
	}
...
static void http_uris_init(struct http_server *hs)
{
	...
	http_register_uri(hs, "", http_gen_cb);
	http_register_uri(hs, "/stream/getvideo", http_stream_getvideo_cb);
	http_register_uri(hs, "/stream/getaudio", http_stream_getaudio_cb);
	http_register_uri(hs, "/snapshot.jpg", http_get_snapshot_cb);
}
